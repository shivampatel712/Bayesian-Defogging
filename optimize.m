function [final_albedo, final_depth] = optimize(I_n, apow, ascale, initial_albedo, initial_depth, partial_type, iters, verbose, albedo_prior_weight, depth_prior_weight, depth_prior_type)
    ...
    u(3*npix+1:end) = +Inf;
    if verbose
        options = optimoptions('fmincon', 'Algorithm', 'interior-point', 'Hessian', {'lbfgs', 16}, 'HessFcn', [], 'HessMult', [], 'GradObj', 'on', 'MaxFunEvals', iters,'TolFun',1e-8,'TolCon',1e-8, 'TolProjCGAbs', 1e-9, 'Display', 'iter');
    else
        options = optimoptions('fmincon', 'HessianApproximation', {'lbfgs', 16}, 'SpecifyObjectiveGradient', true, 'MaxFunctionEvaluations', iters, 'TolProjCGAbs', 1e-9);
    end
